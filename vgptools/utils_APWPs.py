...
#         if len(poles)>2: # this just ensures that dict isn't empty
...
def get_vgps_sampling_from_direction(df, study_label= 'Study',
    ...
    '''
    Takes a DF with the following site information: 
    - Study, site coordinates, mean direction, concentration parameter, mean age and errors) 
    
    First, generates a pseudo-sample from the original dataset using nonparametric random sampling 
    with replacement (bootstrapped sample). Then, draws for each direction a random direction following 
    the kappa concentration parameter and mean direction. Then, it calculates from the random direction 
    a given VGP. 
    The assignation of ages to each pair direction/VGP, it is taken at random from either uniform 
...
def MC_error_prop_ensemble_results(df_vgps_original, n_bst = 100, 
